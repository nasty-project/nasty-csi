...
var _ = Describe("iSCSI Crash Simulation", func() {
	...
	It("should persist data after POD crash (force delete)", func() {
		...
		pvc, err := f.CreatePVC(ctx, framework.PVCOptions{
			Name:             fmt.Sprintf("crash-pvc-iscsi-%d", timestamp),
		...
		pod, err := f.CreatePod(ctx, framework.PodOptions{
			Name:      fmt.Sprintf("crash-pod-iscsi-%d", timestamp),
			PVCName:   pvc.Name,
			MountPath: "/data",
		})
		...
		newPod, err := f.CreatePod(ctx, framework.PodOptions{
			Name:      fmt.Sprintf("crash-pod-iscsi-recovery-%d", timestamp),
			PVCName:   pvc.Name,
			MountPath: "/data",
		})
