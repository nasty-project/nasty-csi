...
var _ = Describe("Snapshot Restore", func() {
	...
	It("should restore from multiple snapshots with correct point-in-time data", func() {
		...
		snapshotClass := "nasty-csi-iscsi-snapshot"
		...
		pvc, err := f.CreatePVC(ctx, framework.PVCOptions{
			Name:             "snapshot-restore-source-iscsi",
		...
		podName := "snapshot-restore-source-pod-iscsi"
		...
		snapshot1 := "snapshot-restore-1-iscsi"
		...
		snapshot2 := "snapshot-restore-2-iscsi"
		...
		restore1PVC := "snapshot-restore-pvc-1-iscsi"
		...
		restore1PodName := "snapshot-restore-pod-1-iscsi"
		...
		restore2PVC := "snapshot-restore-pvc-2-iscsi"
		...
		restore2PodName := "snapshot-restore-pod-2-iscsi"
		...
		if f.Verbose() {
			GinkgoWriter.Printf("[iSCSI] Snapshot restore test completed successfully\n")
