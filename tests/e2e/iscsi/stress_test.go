...
var _ = Describe("Snapshot Stress", func() {
	...
	It("should handle multiple snapshots of the same volume", func() {
		...
		snapshotClass := "nasty-csi-iscsi-snapshot-stress"
		...
		pvcName := "snapshot-stress-source-iscsi"
		...
		podName := "snapshot-stress-pod-iscsi"
		...
		for i := range numSnapshots {
			snapshotName := fmt.Sprintf("snapshot-stress-%d-iscsi", i+1)
			...
			if f.Verbose() {
				GinkgoWriter.Printf("[iSCSI] Snapshot %d/%d created: %s\n", i+1, numSnapshots, snapshotName)
			}
		}
		...
		restore1PVC := "snapshot-stress-restore-1-iscsi"
		...
		restoreLastPVC := "snapshot-stress-restore-last-iscsi"
		...
		restore1PodName := "snapshot-stress-restore-pod-1-iscsi"
		...
		restoreLastPodName := "snapshot-stress-restore-pod-last-iscsi"
		...
		if f.Verbose() {
			GinkgoWriter.Printf("[iSCSI] Successfully created and verified %d snapshots\n", numSnapshots)
		}
	})
})
...
var _ = Describe("Volume Stress", func() {
	...
	It("should handle multiple volumes concurrently", func() {
		...
		for i := range numVolumes {
			...
			go func(index int) {
				defer wg.Done()
				pvcName := fmt.Sprintf("stress-iscsi-pvc-%d", index+1)
		...
		for i := range numVolumes {
			...
			go func(index int) {
				defer wg.Done()
				podName := fmt.Sprintf("stress-iscsi-pod-%d", index+1)
		...
		for _, podName := range podNames {
			...
			if f.Verbose() {
				GinkgoWriter.Printf("[iSCSI] POD %s ready\n", podName)
			}
		}
		...
		if f.Verbose() {
			GinkgoWriter.Printf("[iSCSI] Successfully created and verified %d concurrent volumes\n", numVolumes)
		}
	})
})
