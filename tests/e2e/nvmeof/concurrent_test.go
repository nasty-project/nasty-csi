...
var _ = Describe("NVMe-oF Concurrent Operations", func() {
	...
	It("should handle concurrent PVC+POD creation without race conditions", func() {
		// NVMe-oF uses WaitForFirstConsumer, so we need to create pods
		// along with PVCs to trigger binding
		pvcNames := make([]string, numVolumes)
		podNames := make([]string, numVolumes)
		for i := range numVolumes {
			pvcNames[i] = fmt.Sprintf("concurrent-pvc-nvmeof-%d", i+1)
			podNames[i] = fmt.Sprintf("concurrent-pod-nvmeof-%d", i+1)
		}
	...
	It("should handle sequential POD failover on same PVC", func() {
		// NVMe-oF uses ReadWriteOnce, so only one pod can access at a time
		// This test verifies data persists across pod recreation
		const pvcName = "failover-pvc-nvmeof"
