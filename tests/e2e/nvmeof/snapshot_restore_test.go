...
var _ = Describe("Snapshot Restore", func() {
	...
	It("should restore from multiple snapshots with correct point-in-time data", func() {
		...
		snapshotClass := "nasty-csi-nvmeof-snapshot"
		...
		pvc, err := f.CreatePVC(ctx, framework.PVCOptions{
			Name:             "snapshot-restore-source-nvmeof",
		...
		podName := "snapshot-restore-source-pod-nvmeof"
		...
		snapshot1 := "snapshot-restore-1-nvmeof"
		...
		snapshot2 := "snapshot-restore-2-nvmeof"
		...
		restore1PVC := "snapshot-restore-pvc-1-nvmeof"
		...
		restore1PodName := "snapshot-restore-pod-1-nvmeof"
		...
		By("Deleting restore POD 1 before creating restore 2 (RWO constraint)")
		err = f.K8s.DeletePod(ctx, restore1Pod.Name)
		Expect(err).NotTo(HaveOccurred(), "Failed to delete restore POD 1")
		err = f.K8s.WaitForPodDeleted(ctx, restore1Pod.Name, 2*time.Minute)
		Expect(err).NotTo(HaveOccurred(), "Restore POD 1 was not deleted")

		By("Restoring PVC from second snapshot")
		restore2PVC := "snapshot-restore-pvc-2-nvmeof"
		...
		restore2PodName := "snapshot-restore-pod-2-nvmeof"
		...
		if f.Verbose() {
			GinkgoWriter.Printf("[NVMe-oF] Snapshot restore test completed successfully\n")
