...
var _ = Describe("Snapshot Independence", func() {
	...
	It("should create independent snapshot that survives source deletion", func() {
		...
		sourcePVCName := "snap-indep-src-nvmeof"
		...
		sourcePodName := "snap-indep-src-pod-nvmeof"
		...
		snapshotClassName := "snap-indep-snapclass-nvmeof"
		...
		snapshotName := "snap-indep-snap-nvmeof"
		...
		if sourceExists {
			GinkgoWriter.Printf("[NVMe-oF] WARNING: Source subvolume %s still exists after PVC deletion\n", sourceDatasetPath)
			Fail(fmt.Sprintf("Source subvolume %s could not be deleted", sourceDatasetPath))
		} else {
			GinkgoWriter.Printf("[NVMe-oF] SUCCESS: Source subvolume %s was deleted\n", sourceDatasetPath)
		}
		...
		GinkgoWriter.Printf("[NVMe-oF] SUCCESS: Snapshot is independent — survived source volume deletion\n")
	})
})
