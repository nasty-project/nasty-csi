...
var _ = Describe("Detached Snapshot Independence", func() {
	...
	It("should create truly independent detached snapshot (no ZFS clone dependency)", func() {
		...
		sourcePVCName := "detached-indep-src-nvmeof"
		...
		sourcePodName := "detached-indep-src-pod-nvmeof"
		...
		GinkgoWriter.Printf("[NVMe-oF] Source PV: %s\n", sourcePVName)
		...
		GinkgoWriter.Printf("[NVMe-oF] Source ZFS dataset: %s\n", sourceDatasetPath)

		By("Writing test data to source volume")
		testData := fmt.Sprintf("Independence Test Data - NVMe-oF - %d", time.Now().UnixNano())
		...
		snapshotName := "detached-indep-snap-nvmeof"
		...
		GinkgoWriter.Printf("[NVMe-oF] Snapshot handle: %s\n", contentInfo.SnapshotHandle)
		...
		GinkgoWriter.Printf("[NVMe-oF] CSI snapshot name from handle: %s\n", csiSnapshotName)
		...
		GinkgoWriter.Printf("[NVMe-oF] Detached snapshot dataset path: %s (exists: %v)\n", detachedDatasetPath, exists)
		...
		if isClone {
			GinkgoWriter.Printf("[NVMe-oF] FAILURE: Detached snapshot IS a clone! Origin: %s\n", origin)
			Fail(fmt.Sprintf("Detached snapshot %s is a ZFS clone with origin %s - it should be independent", detachedDatasetPath, origin))
		} else {
			GinkgoWriter.Printf("[NVMe-oF] SUCCESS: Detached snapshot is NOT a clone - it is truly independent\n")
		}
		...
		if sourceExists {
			GinkgoWriter.Printf("[NVMe-oF] WARNING: Source dataset %s still exists after PVC deletion\n", sourceDatasetPath)
			Fail(fmt.Sprintf("Source dataset %s could not be deleted - likely due to clone dependency", sourceDatasetPath))
		} else {
			GinkgoWriter.Printf("[NVMe-oF] SUCCESS: Source dataset %s was deleted\n", sourceDatasetPath)
		}
		...
		restoredPVCName := "detached-indep-restored-nvmeof"
		...
		restoredPodName := "detached-indep-restored-pod-nvmeof"
		...
		if f.Verbose() {
			GinkgoWriter.Printf("[NVMe-oF] Test PASSED: Detached snapshot is truly independent\n")
