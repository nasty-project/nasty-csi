...
var _ = Describe("SMB Crash Simulation", func() {
	...
	It("should persist data after POD crash (force delete)", func() {
		...
		pvc, err := f.CreatePVC(ctx, framework.PVCOptions{
			Name:             fmt.Sprintf("crash-pvc-smb-%d", timestamp),
		...
		pod, err := f.CreatePod(ctx, framework.PodOptions{
			Name:      fmt.Sprintf("crash-pod-smb-%d", timestamp),
			PVCName:   pvc.Name,
			MountPath: "/data",
		})
		...
		newPod, err := f.CreatePod(ctx, framework.PodOptions{
			Name:      fmt.Sprintf("crash-pod-smb-recovery-%d", timestamp),
			PVCName:   pvc.Name,
			MountPath: "/data",
		})
