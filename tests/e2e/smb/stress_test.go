...
var _ = Describe("Snapshot Stress", func() {
	...
	It("should handle multiple snapshots of the same volume", func() {
		...
		snapshotClass := "nasty-csi-smb-snapshot-stress"
		...
		pvcName := "snapshot-stress-source-smb"
		...
		podName := "snapshot-stress-pod-smb"
		...
		for i := range numSnapshots {
			snapshotName := fmt.Sprintf("snapshot-stress-%d-smb", i+1)
			...
			if f.Verbose() {
				GinkgoWriter.Printf("[SMB] Snapshot %d/%d created: %s\n", i+1, numSnapshots, snapshotName)
			}
		}
		...
		restore1PVC := "snapshot-stress-restore-1-smb"
		...
		restoreLastPVC := "snapshot-stress-restore-last-smb"
		...
		restore1PodName := "snapshot-stress-restore-pod-1-smb"
		...
		restoreLastPodName := "snapshot-stress-restore-pod-last-smb"
		...
		if f.Verbose() {
			GinkgoWriter.Printf("[SMB] Successfully created and verified %d snapshots\n", numSnapshots)
		}
	})
})
...
var _ = Describe("Volume Stress", func() {
	...
	It("should handle multiple volumes concurrently", func() {
		...
		for i := range numVolumes {
			...
			go func(index int) {
				defer wg.Done()
				pvcName := fmt.Sprintf("stress-smb-pvc-%d", index+1)
		...
		for _, pvcName := range pvcNames {
			...
			if f.Verbose() {
				GinkgoWriter.Printf("[SMB] PVC %s bound\n", pvcName)
			}
		}
		...
		for i := range numVolumes {
			...
			go func(index int) {
				defer wg.Done()
				podName := fmt.Sprintf("stress-smb-pod-%d", index+1)
		...
		for _, podName := range podNames {
			...
			if f.Verbose() {
				GinkgoWriter.Printf("[SMB] POD %s ready\n", podName)
			}
		}
		...
		if f.Verbose() {
			GinkgoWriter.Printf("[SMB] Successfully created and verified %d concurrent volumes\n", numVolumes)
		}
	})
})
