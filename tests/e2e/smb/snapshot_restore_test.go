...
var _ = Describe("Snapshot Restore", func() {
	...
	It("should restore from multiple snapshots with correct point-in-time data", func() {
		...
		pvc, err := f.CreatePVC(ctx, framework.PVCOptions{
			Name:             "snapshot-restore-source-smb",
		...
		podName := "snapshot-restore-source-pod-smb"
		...
		snapshot1 := "snapshot-restore-1-smb"
		...
		snapshot2 := "snapshot-restore-2-smb"
		...
		restore1PVC := "snapshot-restore-pvc-1-smb"
		...
		restore1PodName := "snapshot-restore-pod-1-smb"
		...
		restore2PVC := "snapshot-restore-pvc-2-smb"
		...
		restore2PodName := "snapshot-restore-pod-2-smb"
		...
		if f.Verbose() {
			GinkgoWriter.Printf("[SMB] Snapshot restore test completed successfully\n")
