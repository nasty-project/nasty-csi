...
var _ = Describe("iSCSI Encryption", func() {
	...
	Context("Basic Operations", func() {
		It("should provision encrypted volume and perform I/O", func() {
			...
			pod, err := f.CreatePod(ctx, framework.PodOptions{
				Name:      "encrypted-iscsi-pod-basic",
				PVCName:   pvc.Name,
				MountPath: "/data",
			})
			Expect(err).NotTo(HaveOccurred())
			...
			testData := "Encrypted iSCSI Test Data"
		...
		It("should provision encrypted volume with custom algorithm (AES-128-CCM)", func() {
			...
			pod, err := f.CreatePod(ctx, framework.PodOptions{
				Name:      "encrypted-iscsi-pod-aes128",
				PVCName:   pvc.Name,
				MountPath: "/data",
			})
			Expect(err).NotTo(HaveOccurred())
			...
			testData := "AES-128-CCM Encrypted iSCSI Data"
			_, err = f.K8s.ExecInPod(ctx, pod.Name, []string{
				"sh", "-c", fmt.Sprintf("echo '%s' > /data/test.txt && sync", testData),
			})
			Expect(err).NotTo(HaveOccurred())
	...
	Context("Volume Expansion", func() {
		It("should expand encrypted volume (offline)", func() {
			...
			By("Creating POD")
			pod, err := f.CreatePod(ctx, framework.PodOptions{
				Name:      "encrypted-iscsi-pod-expand",
				PVCName:   pvc.Name,
				MountPath: "/data",
			})
			Expect(err).NotTo(HaveOccurred())
			...
			By("Deleting POD for offline expansion (iSCSI requirement)")
			...
			// iSCSI requires NodeExpansionRequired=true, meaning the PVC capacity
			...
			pod2, err := f.CreatePod(ctx, framework.PodOptions{
				Name:      "encrypted-iscsi-pod-expand2",
				PVCName:   pvc.Name,
				MountPath: "/data",
			})
			Expect(err).NotTo(HaveOccurred())
		...
		It("should expand encrypted volume (inline)", func() {
			...
			By("Creating PVC")
			pvc, err := f.CreatePVC(ctx, framework.PVCOptions{
				Name:             "encrypted-iscsi-expand-inline",
				StorageClassName: scName,
				Size:             "1Gi",
				AccessModes:      []corev1.PersistentVolumeAccessMode{corev1.ReadWriteOnce},
			})
			...
			By("Creating POD")
			pod, err := f.CreatePod(ctx, framework.PodOptions{
				Name:      "encrypted-iscsi-pod-expand-inline",
				PVCName:   pvc.Name,
				MountPath: "/data",
			})
			Expect(err).NotTo(HaveOccurred())
	...
	Context("Snapshots", func() {
		It("should create snapshot from encrypted volume and restore", func() {
			...
			pvc, err := f.CreatePVC(ctx, framework.PVCOptions{
				Name:             "encrypted-iscsi-snapshot-source",
				StorageClassName: scName,
				Size:             "1Gi",
				AccessModes:      []corev1.PersistentVolumeAccessMode{corev1.ReadWriteOnce},
			})
			...
			pod, err := f.CreatePod(ctx, framework.PodOptions{
				Name:      "encrypted-iscsi-snapshot-pod",
				PVCName:   pvc.Name,
				MountPath: "/data",
			})
			Expect(err).NotTo(HaveOccurred())
			...
			testData := "iSCSI Snapshot Test Data"
			_, err = f.K8s.ExecInPod(ctx, pod.Name, []string{
			...
			snapshotName := "encrypted-iscsi-snapshot"
			...
			restoredPVCName := "encrypted-iscsi-restored"
			...
			restoredPod, err := f.CreatePod(ctx, framework.PodOptions{
				Name:      "encrypted-iscsi-restored-pod",
	...
	Context("Volume Cloning", func() {
		It("should clone encrypted volume", func() {
			...
			sourcePVC, err := f.CreatePVC(ctx, framework.PVCOptions{
				Name:             "encrypted-iscsi-clone-source",
				StorageClassName: scName,
				Size:             "1Gi",
				AccessModes:      []corev1.PersistentVolumeAccessMode{corev1.ReadWriteOnce},
			})
			...
			sourcePod, err := f.CreatePod(ctx, framework.PodOptions{
				Name:      "encrypted-iscsi-clone-source-pod",
			...
			testData := "Encrypted iSCSI Clone Source Data"
			...
			clonePVCName := "encrypted-iscsi-clone"
			...
			clonePod, err := f.CreatePod(ctx, framework.PodOptions{
				Name:      "encrypted-iscsi-clone-pod",
	...
	Context("Persistence", func() {
		It("should persist data across POD restart", func() {
			...
			pod1, err := f.CreatePod(ctx, framework.PodOptions{
				Name:      "encrypted-iscsi-persist-pod1",
				PVCName:   pvc.Name,
				MountPath: "/data",
			})
			Expect(err).NotTo(HaveOccurred())
			...
			testData := "Persistent Encrypted iSCSI Data"
			...
			pod2, err := f.CreatePod(ctx, framework.PodOptions{
				Name:      "encrypted-iscsi-persist-pod2",
				PVCName:   pvc.Name,
				MountPath: "/data",
			})
			Expect(err).NotTo(HaveOccurred())
	...
	Context("Block Mode", func() {
		It("should provision encrypted volume in block mode", func() {
			...
			pod, err := f.K8s.CreatePod(ctx, framework.PodOptions{
				Name:       "encrypted-iscsi-block-pod",
				PVCName:    pvc.Name,
