...
- **MAJOR.MINOR.PATCH** (e.g., `v0.16.0`)
- Tags are prefixed with `v` (e.g., `v0.16.0`, `v1.0.0`)
...
2. **Git describe** - For non-tagged commits, format is `v0.16.0-3-gabc1234` (3 commits after v0.16.0)
...
|-----|-------------|-----------|
| `v0.16.0` | Exact version | Immutable |
...
|------------------|-------|
| `version` | `0.16.0` (chart version, no `v` prefix) |
| `appVersion` | `v0.16.0` (app version, with `v` prefix) |

### Image Tag Resolution

The Helm chart resolves the image tag in this order:

1. **Explicit override**: `--set image.tag=v0.16.0`
2. **Chart's appVersion**: Automatically uses `v0.16.0` when installing `--version 0.16.0`
